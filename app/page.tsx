...
export default async function HomePage() {
  ...
  return (
    <div className="py-4">
      <Typography variant="h4" component="h2" align="center" sx={{ marginBottom: '16px' }}>Blog Posts</Typography>
      
      <div className="space-y-3">
        {posts.map((post: any) => (
          <Card key={post.slug} sx={{ marginBottom: '12px' }}>
            <CardContent>
              <MuiLink component={LinkBehavior} href={`/${post.slug}`} underline="hover" variant="h6" sx={{ marginBottom: '8px' }}>
                {post.title}
              </MuiLink>
              <Typography variant="body2" color="text.secondary" sx={{ marginBottom: '8px' }}>{post.description}</Typography>
